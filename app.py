...
# Dirección única para el nodo actual
...
def consensus():
    ...
    def get_neighbour_chains():
        ...
        for node_address in blockchain.nodes:
            resp = requests.get(
                node_address + url_for('get_full_chain')).json()
            chain = resp['chain']
    ...
    # Obtenemos la cadena más larga
    longest_chain = max(neighbour_chains, key=len)

    # Si nuestra cadena es la más larga, no hacemos nada
    if len(blockchain.chain) >= len(longest_chain):
        ...
        # De lo contrario, la almacenamos
